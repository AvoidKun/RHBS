<script setup>
import { ref } from 'vue'
import { adminPasswordUpdateService } from '@/api/admin.js'
...
const updatePassword = async () => {
    ...
    loading.value = true;
    try {
        let result = await adminPasswordUpdateService(formattedData)
        if (result.code === 0) {
            ElMessage.success(result.msg ? result.msg : '修改成功')
            passwordInfo.value = { oldPwd: '', newPwd: '', rePwd: '' }
            router.push('/login')
        } else {
            ElMessage.error(result.msg ? result.msg : '原密码错误')
        }
    } catch (error) {
